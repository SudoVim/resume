import React from "react";
import { Essay } from "components/utils";

export function Summary() {
  return (
    <Essay>
      In my thirteen-year journey as a software engineer, I've honed a knack for
      transforming even the vaguest requirements into impeccably designed,
      implemented, and maintained products. My career path has seen me wear
      multiple hats, seamlessly transitioning from individual contributor roles
      in quality assurance, DevOps, and programming to the helm of technical
      leadership. As a versatile leader, I not only grasp the responsibilities
      of every team member but stand ready to step in at a moment's notice,
      ensuring project continuity and success.
    </Essay>
  );
}
